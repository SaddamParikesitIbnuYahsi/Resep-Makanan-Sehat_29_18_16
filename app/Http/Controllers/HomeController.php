<?php

namespace App\Http\Controllers;

use Illuminate\Http\Request;

class HomeController extends Controller
{
    public function index(){
    $Breakfasts = Breakfast::all();
    $Dinners = Dinner::all();

    return view("index", compact('breakfast','Breakfasts'));
    }   
    public function breakfast(){
        return view('breakfast');
    }  
    public function dinner(){
        return view('dinner');
    }  
    public function sopbening(){
        return view('sopbening');
    }  
    public function dori(){
        return view('dori');
    }  
    public function sapi(){
        return view('sapi');
    }  
    public function pecel(){
        return view('pecel');
    }  
    public function ayap(){
        return view('ayap');
    }  
    public function blackpep(){
        return view('blackpep');
    }  
    public function suwir(){
        return view('suwir');
    }  
    public function bola(){
        return view('bola');
    }  
    public function pasta(){
        return view('pasta');
    }  
    public function oatmeal(){
        return view('oatmeal');
    }  
    public function pancake(){
        return view('pancake');
    }  
    public function smoothies(){
        return view('smoothies');
    }  
    public function capcay(){
        return view('capcay');
    }  
    public function sandwich(){
        return view('sandwich');
    }  
    public function salad(){
        return view('salad');
    }  
    public function tahu(){
        return view('tahu');
    }  
    public function omlet(){
        return view('omlet');
    }  
    public function sda(){
        return view('sda');
    }  
    public function cu(){
        return view('cu');
    }  
    public function gurame(){
        return view('gurame');
    }  
    public function nasi(){
        return view('nasi');
    }  
    public function pepes(){
        return view('pepes');
    }  
    public function steak(){
        return view('steak');
    }  
    public function sate(){
        return view('sate');
    }  
}
